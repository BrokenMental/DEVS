...
public class Log {
    ...
    private static final int TAG_WIDTH = 20;
    ...
    public static void e(String tag, String message) {
        ...
        System.err.println(LEVEL_ERROR + makeTag(tag) + message);                
    }
    
    /** Log a warning message. */
    public static void w(String tag, String message) {
        if (sLevel < WARN) return;
        System.err.println(LEVEL_WARN + makeTag(tag) + message);
    }
    
    /** Log a debug message. */
    public static void d(String tag, String message) {
        if (sLevel < DEBUG) return;
        System.err.println(LEVEL_DEBUG + makeTag(tag) + message);
    }
    
    /** Log an informative message. */
    public static void i(String tag, String message) {
        if (sLevel < INFO) return;
        System.out.println(LEVEL_INFO + makeTag(tag) + message);
    }
    
    /** Log a verbose message. */
    public static void v(String tag, String message) {
        if (sLevel < VERBOSE) return;
        System.out.println(LEVEL_VERBOSE + makeTag(tag) + message);
    }
    
    /** Make the tag the width of the tag column. */
    private static String makeTag(String tag) {
        return String.format("%-" + TAG_WIDTH + "s", tag) + " " + COL_DELIM + " ";
    }
}
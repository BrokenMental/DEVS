...
/**
 * An application that displays the execution of a simulation employing a
 * coupled devs model.
 * 
 * @author Jeff Mather
 */
public class SimView {
	/**
	 * A label that displays whether the viewer is currently executing a step
	 * (or running multiple steps) of a simulation, or is ready for the next
	 * user command.
	 */
	...
	/**
	 * Whether to always show couplings between components, or just when the
	 * mouse is over one of them.
	 */
	protected boolean alwaysShowCouplings = false;

	protected boolean showSpecificCouplings = false;
	...
	protected boolean freezeCoupling = false; // saurabh

	protected boolean selectiveCouple = false;// saurabh
	...
	/**
	 * The name given to the default digraph that this sim-view uses to wrap
	 * single atomic models that are to be viewed.
	 */
	...
	/**
	 * The list of model package names from which models may be selected for
	 * simulation.
	 */
	...
	/**
	 * The panel which displays the components of the current model, along with
	 * the scrollpane containing that panel.
	 */
	...
	/**
	 * A label that displays how much time has elapsed in the simulation so far.
	 */
	 ...
	 * choices available for its value, as well as the label that displays its
	 * current value, and the slider that adjusts that value.
	 */
	protected double[] realTimeFactors = { .0001, .001, .01, .1, .5, 1, 5, 10,
			100, 1000, 10000 };
	...
	/**
	 * A combo-box that lists all the packages of models from which the user may
	 * select a model to simulate.
	 */
	protected JComboBox packagesBox;

	/**
	 * A combo-box that lists all the models on which the user may choose to run
	 * a simulation.
	 */
	...
	/**
	 * The path (relative to the working directory) at which are located the
	 * java model packages, in which model classes may be found.
	 */
	protected String modelsPath;

	/**
	 * The path (relative to the working directory) at which are located the
	 * java source files of the model packages, that this sim-view may modify
	 * those files to save model layouts.
	 */
	protected String sourcePath;

	/**
	 * The current models package selected, from which model classes may be
	 * chosen for simulation.
	 */
	protected String modelsPackage;

	/**
	 * The name of the last model viewed by this program. This is saved to a
	 * file so that the same model may be automatically loaded the next time the
	 * program is run.
	 */
	...
	/**
	 * by saurabh Constructs an exploded window for the blackbox digraph
	 */
	public SimView(ViewableDigraph dg, String modelsPackage, String modelClass,
			boolean level, double prevSimTime) {
	...
	protected void constructUI() {
		...
		final JFrame frame = mainFrame = new JFrame(
				"DEVSJAVA Simulation Viewer");
		...
		combo.setMaximumSize(new Dimension(combo.getMaximumSize().width, combo
		...
		modelsBox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (!showLevel) {
					...
					// System.out.println("The data is package 2
					// <"+modelsPackage+"> class <"+className+">
					// model<"+model.getName()+">");
		...
		/*
		 * JScrollPane scrollPane = modelViewScrollPane = new
		 * JScrollPane(modelView);
		 */
		JScrollPane scrollPane = modelViewScrollPane = new JScrollPane(
				modelView);
		...
		// be moved around
		Dimension size = new Dimension(label.getFontMetrics(label.getFont())
				.stringWidth("stepping") + 10, label.getMinimumSize().height);
		...
		checkBox.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent e) {
				alwaysShowCouplings = (e.getStateChange() == ItemEvent.SELECTED);
				modelView.repaint();
			}
		});
		...
		// adjust tooltip delay settings to make tooltips show up immediately
		// and
		// not go away
	...
	// ///////////////////////////////////////
	/**
	 * By Saurabh Constructs the UI of the child window of the blackbox digraph.
	 */
	protected void constructUI_SM(ViewableDigraph dg, String modelsPackage,
			String modelClass, double prevSimTime) {
		final JFrame frame;
		// create the app frame
		...
		if (!showLevel) { // ////////if its a child or an updated parent
			...
			Dimension size = new Dimension(
					label.getFontMetrics(label.getFont()).stringWidth(
							"stepping") + 10, label.getMinimumSize().height);
			label.setMinimumSize(size);
			label.setMaximumSize(size);
			label.setPreferredSize(size);
			...
			// this will get the initial real-time-factor value shown by the
			// label
			realTimeFactor.set(realTimeFactor.get());
			...
			// when the smodelViewtep button is clicked
			button.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					if (coordinator == null) {
						return;
	 ...
	 */
	public class ModelView extends JLayeredPane implements
			TunableCoordinator.Listener, TunableCoupledCoordinator.Listener,
			ViewableAtomicSimulator.Listener,
			ViewableAtomicSimulator.TimeScaleKeeper, variableStructureViewer {
		/**
		 * The thread responsible for moving the currently displayed content
		 * views around this model-view.
		 */
		protected MoveContentViewThread moveContentViewThread;

		/**
		 * A map of the movement objects of the content views currently being
		 * moved around this model-view.
		 */
		protected Map contentViewMovementMap = new HashMap();

		/**
		 * Whether this view should display the couplings between the components
		 * in the model.
		 */
		 ...
		 * the lines meant to represent the couplings between the components in
		 * the model.
		 */
		...
		protected List frozenCouplings = new ArrayList(); // by saurabh

		protected ViewableComponent component; // by saurabh

		protected ViewableComponent componentSrc; // by saurabh

		protected ViewableComponent componentDest; // by saurabh

		/**
		 * The views on the components of the model this view is displaying.
		 */
		...
		/**
		 * A map of the paths contents are taking as they travel from their
		 * source to their destination. The key used is based of the content's
		 * latest step in its path, meaning it is composed of the content as
		 * well as the step's component name.
		 */
		protected Map contentPathMap = new HashMap();

		/**
		 * Whether or not this model-view should execute just one iteration of
		 * the simulation at a time, and wait until the user tells it to execute
		 * the next iteration, or if it should just keep on executing iterations
		...
		public void removeView(ComponentView view, JComponent parent) {
			...
			// view:"+view.getViewableComponent().getName());
		}

		/**
		 * Informs this model view that the user has injected an input into one
		 * of the components being viewed.
		 */
		...
		/**
		 * See genDevs.simulation.realTime.TunableCoordinator.Listener interface
		 * method.
		 */
		...
		/**
		 * Returns the time-scale (or, real time factor), currently in use by
		 * this sim-view's coordinator for the current model.
		 */
		...
		/**
		 * Informs this model-view that a single iteration of the simulation is
		...
		/**
		 * Informs this model-view that a run of simulation iterations (versus
		 * individual iteration steps) is about to occur.
		 */
		 ...
		 * See ViewableAtomicSimulator.Listener interface method.
		 */
		public void contentOutputted(content content, devs source,
		...
		public void couplingAddedToContentPath(content oldContent,
				devs destComponent, String destPortName, content newContent,
			...
			ContentPathKey key = new ContentPathKey(oldContent,
					sourceComponentName);
			...
			// has one)
			ContentViewMovement movement = (ContentViewMovement) contentViewMovementMap
					.remove(path);
		...
		public Dimension getPreferredSize() {
			...
			return new Dimension(size.width + modelBorderWidth * 2, size.height
					+ modelBorderWidth * 2);
		}

		/**
		 * Together, these force this model-view's size to be its preferred
		 * size.
		 */
		...
		/**
		 * Removes from this model-view each of the content-views it is
		 * displaying.
		 */
		...
		/**
		 * Informs this model-view that the mouse has entered a port-box on one
		...
		public void mouseEnteredPort(String myPortName) {
			showSpecificCouplings = true;
			// showCouplings = true;
			portName = myPortName;
			repaint();
		}
		...
		/**
		 * Informs this model-view that the mouse has exited a port-box on one
		 * of the model's components.
		 */
		public void mouseExitedPort() {
			showCouplings = false;
			showSpecificCouplings = false;
			repaint();
		}

		// by saurabh

		public void mousePressedOnPort(String myPortName,
				ViewableComponent component_) {
			selectiveCouple = true;
			portSrc = myPortName;
			componentSrc = component_;
			s.s("port: " + portSrc + "    componentSrc: " + componentSrc);
			// repaint();
		}

		public void mouseReleasedOnPort(String myPortName,
				ViewableComponent component_) {
			componentDest = component_;
			portDest = myPortName;
			s.s("port: " + portDest + "    componentDest: " + componentDest);
			repaint();
		}

		public void mouseClickedTwiceOnPort(String myPortName,
				ViewableComponent component_) {
			freezeCoupling = true;
		...
		public void clearAllFrozenCouplings() {
			freezeCoupling = false;
			selectiveCouple = false;
			frozenCouplings.clear(); // clear all the portNames in this array
			repaint();
		}

		public void clearFrozenCoupling(String myPortName) {
			portName = myPortName;
			frozenCouplings.remove(portName);
			repaint();
		}
		...
		/**
		 * An object associating a content-view with various information about
		 * its movement along a path within the model-view.
		 */
		protected class ContentViewMovement {
			...
			/**
			 * The index of the current step on the path along which the content
			 * view is moving.
			 */
			protected int currentStepIndex;

			/**
			 * The x and y offsets to take with each step along the current
			 * line.
			 */
			protected double dx, dy;

			/**
			 * The current location of this view along the current line, with
			 * more precision than just integer coordinates, so that this view
		...
		/**
		 * Moves each content view in its list along a series of steps (i.e.
		 * couplings) to its final destination port.
		 */
		protected class MoveContentViewThread extends Thread {
			 ...
			 * 
			 * @param movement
			 *            The content-view movement to execute.
			 */
			...
			protected void moveContentViews() {
				...
				for (int i = 0; i < movements.size(); i++) {
					ContentViewMovement movement = (ContentViewMovement) movements
							.get(i);
					...
					if (movement.movesDone == 0) {
						...
						// is hidden
						ContentPathStep step = (ContentPathStep) movement.path
								.get(movement.currentStepIndex);
						if (step.view.getViewableComponent().isHidden()) {
							// move on to the next step
							movement.currentStepIndex++;

							// if this was the last step in the movement
							if (movement.currentStepIndex >= movement.path
									.size()) {
						...
						if (movement.currentStepIndex == 0
							...
							Point start = modelView
									.getLocation((JComponent) step.view);
							PointUtil.translate(start, step.view
									.getPortLocation(step.portName));
							int viewX = start.x - view.getWidth() / 2, viewY = start.y
									- view.getHeight() / 2;
						...
						Point start = new Point((int) movement.location.x
								+ view.getWidth() / 2,
								(int) movement.location.y + view.getHeight()
										/ 2);

						// detm the finish location for this step
						Point finish = modelView
								.getLocation((JComponent) step.view);
						PointUtil.translate(finish, step.view
								.getPortLocation(step.portName));

						// compute the x and y offsets to take with each step
						// along the line
						movement.location = new Point2D.Double(start.x, start.y);
						double angle = Math.atan2(finish.y - start.y, finish.x
								- start.x);
						final int speed = 2;
						movement.dx = speed * Math.cos(angle);
						movement.dy = speed * Math.sin(angle);

						// detm how many moves it will take to get to the
						// destination
						Point2D.Double location = movement.location;
						movement.movesRequired = (int) Math.rint(location
								.distance(finish)
								/ speed);
						movement.movesDone = 0;

						// start the content view centered over the source port
						// location
			 ...
			 * 
			 * @param movement
			 *            The movement object to discard.
			 */
		...
		protected class CouplingsPanel extends JPanel {
			...
			public void paintSelectedCoupling(Graphics g) {
				...
				for (int i = 0; i < couplings.size(); i++) {
					...
					// coupling are hidden
					if (coupling.sourceView.getViewableComponent().isHidden()
							|| coupling.destView.getViewableComponent()
									.isHidden()) {
						// don't draw this coupling
						continue;
					}
					if (coupling.sourcePortName.equals(portSrc)
							&& coupling.destPortName.equals(portDest)) {

						// ViewableComponent comp
						// =(ViewableComponent)((coupling.sourceView).getViewableComponent());
						// if(comp.getName().equals(component.getName())){
						// coupling.printCoupling();
						frozenCouplings.add(coupling);

						// }
					}
				}

				for (int i = 0; i < frozenCouplings.size(); i++) {
					Coupling coupling = (Coupling) frozenCouplings.get(i);
					// detm the pixel location within this model-view
					// of the source port of the coupling
					Point source = ModelView.this
							.getLocation((JComponent) coupling.sourceView);
					PointUtil.translate(source, coupling.sourceView
							.getPortLocation(coupling.sourcePortName));

					// detm the pixel location within this model-view
					// of the destination port of the coupling
					Point dest = ModelView.this
							.getLocation((JComponent) coupling.destView);
					PointUtil.translate(dest, coupling.destView
							.getPortLocation(coupling.destPortName));
			...
			// by saurabh
			public void paintFreezeCoupling(Graphics g) {
				...
				for (int i = 0; i < couplings.size(); i++) {
					...
					if (coupling.sourcePortName.equals(portName)) {

						ViewableComponent comp = (ViewableComponent) ((coupling.sourceView)
			...
			// /////////////bpz
			public void paintSpecific(Graphics g) {
				...
				for (int i = 0; i < couplings.size(); i++) {
					...
					if (coupling.sourceView.getViewableComponent().isHidden()
							|| coupling.destView.getViewableComponent()
									.isHidden()
					// || !coupling.sourcePortName.equals(portName)
					// || !coupling.destPortName.equals(portName)
					) {
						// don't draw this coupling
						continue;
					}
			...
			public void paint(Graphics g) {
				...
				if (selectiveCouple) {
					paintSelectedCoupling(g);
				}
				if (freezeCoupling) {
					paintFreezeCoupling(g);
					// return;
				}

				if (showSpecificCouplings) {
					paintSpecific(g);
					return;
				}
		...
		/**
		 * Returns the location of the given descendant component within this
		 * view.
		 * 
		 * @param component
		 *            The descendent component whose location is desired.
		 * @return The location of the component, relative to the upper-left
		 *         corner of this view.
		 */
		...
		public void savingModelViewCouplingsRemove(ViewableComponent iod,
			...
			ViewableDigraph parent = (ViewableDigraph) ((devs) iod).getParent();
			...
			while (it.hasNext()) {
				...
				// model = (ViewableDigraph)coordinator.getModel(); //Xiaolin
				// Hu, March, 2006
				...
				// s.s("Saving the coupling:
				// "+src+"----"+(String)cs.getValue()+"----"+dst+"----"+(String)cd.getValue());

				this.couplingRemoved(source, (String) cs.getValue(), dest,
						(String) cd.getValue());
		...
		public void restoreInternalCouplings(ViewableDigraph digraph,
				couprel cr_) {
			couprel mc = cr_;
			Iterator it = mc.iterator();
		...
		public void couplingRemoved(IODevs src, String p1, IODevs dest,
				String p2) {
			String srcName, destName;
			ViewableDigraph parent;
			for (int i = 0; i < couplings.size(); i++) {
				Coupling coupling = (Coupling) couplings.get(i);
				srcName = (coupling.sourceView).getViewableComponent()
						.getName();
				ViewableComponent srcView = (coupling.sourceView)
						.getViewableComponent();
				...
				ViewableComponent destView = (coupling.destView)
						.getViewableComponent();
		...
		public void destroyModelView(ViewableDigraph model,
			...
			while (i.hasNext()) {
				...
				if (component instanceof ViewableDigraph) {
					...
					destroyModelView((ViewableDigraph) component, model
							.getView());
				}
				// need to remove the coupling connected to that component from
				// the CouplingsPanel
				int csize = couplings.size(); // the size of the couplings
				for (int j = csize - 1; j >= 0; j--) { // a special way to
														// check all the
														// elements in the
														// Arraylist while
														// removing element
					...
					srcName = (coupling.sourceView).getViewableComponent()
							.getName();
					destName = (coupling.destView).getViewableComponent()
							.getName();
		...
		/**
		 * Creates an atomic-view for each viewable atomic in the given digraph
		 * model, as well as for those recursively found contained within in the
		 * given model's children models. Each new atomic-view is added to the
		 * model-view.
		 * 
		 * @param model
		 *            The model for whose atomic components views are to be
		 *            created.
		 * @param parent
		 *            The parent component to which the given model's views are
		 *            to be added.
		 */
		public void createViews(ViewableDigraph model, JComponent parent) {
			...
			if (!model.isBlackBox()) {
				...
				if (!usePrevModelLayout) {
					try {
						// if the override layout method doesn't get the layout
						// done
			...
			// if the given digraph is the top-level digraph in the model being
			// viewed
			if (parent == modelView) {
			...
			while (i.hasNext()) {
				...
				if (component instanceof ViewableAtomic) {
					...
					if (model.getName().equals(wrapperDigraphName)) {
						...
						view1.setLocation(modelViewScrollPane.getWidth() / 2
								- view.getWidth() / 2, modelViewScrollPane
								.getHeight()
								/ 2 - view.getHeight() / 2);
					}
				}
	...
	/**
	 * Adds the names of all java class files (which are assumed to be compiled
	 * devs java models) in the models-path to the given combo-box.
	 * 
	 * @param box
	 *            The combo-box to which to add the model names.
	 */
	protected void populateModelsBox(JComboBox box) {
		...
		while (i.hasNext()) {
			...
			box.addItem(name.substring(0, name.length() - extension.length()));
		}
	}

	/**
	 * Creates an instance of the model class of the given name (including the
	 * package name). Also, creates the views and coordinator for the model
	 * instance.
	 * 
	 * @param name
	...
	protected void useModelClass(String name) {
		...
		if (!showLevel) {
			...
			if (instance instanceof ViewableAtomic) {
				// wrap the instance in a digraph, and have the digraph be our
				// model
	 ...
	 * added by Saurabh
	 */
	public void useModelClass_SM(ViewableDigraph dg, String className,
			double prevSimTime)
	// protected void useModelClass_SM()
	{
		...
		ncoordinator.initialize(prevSimTime); // /get the prev clock from the
												// earliee simView
	...
	/**
	 * A wrapper for a clock value, changes to which must be accompanied by
	 * other actions.
	 */
	...
	protected class RealTimeFactor {
		 ...
		 * The value being wrapped.
		 */
		private double realTimeFactor = realTimeFactors[realTimeFactors.length / 2];
	...
	/**
	 * Adds the names of all the user-specified java packages containing models
	 * to the given combo-box.
	 * 
	 * @param box
	 *            The combo-box to which to add the model names.
	 */
	...
	protected class ConfigureDialog extends JDialog {
		...
		protected void constructUI() {
			...
			JLabel label = new JLabel(
					"Path to packages of model classes (from current folder)");
			...
			label = new JLabel(
					"Path to packages of model source files (from current folder)");
			label.setAlignmentX(0.0f);
			main.add(label);
			...
			button.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if (modelsPath.endsWith("/")) {
						// remove it
						modelsPath = modelsPath.substring(0, modelsPath
								.length() - 1);
					}
					...
					StringReader stringReader = new StringReader(packagesArea
							.getText());
	...
	/**
	 * A coupling between a source port on one devs component view, and a
	...
	protected class Coupling {
		...
		public void printCoupling() {
			...
			System.out.println("Inside coupling: source:<"
					+ sourceView.getViewableComponent().getLayoutName()
					+ "> <sourcePort:" + sourcePortName + "> destination:<"
					+ destView.getViewableComponent().getLayoutName()
	 ...
	 * 
	 * @param model
	 *            The model for which to find all the couplings.
	 */
	...
	/**
	 * Determines all the couplings for which a port on the given component is
	 * the source, and adds those couplings to the model-view for display.
	 * 
	 * @param comp
	 *            The devs component whose couplings are to be found.
	 */
	...
	/**
	 * Determines all the couplings for which a port from the given list (on the
	 * given component) is the source, and adds those couplings to the
	 * model-view for display.
	 * 
	 * @param comp
	 *            The devs component to which the ports belong.
	 * @param portNames
	 *            The list of port names on the above component to look for
	 *            couplings.
	 */
	protected void detmCouplings(ViewableComponent comp, List portNames) {
		...
		for (int i = 0; i < portNames.size(); i++) {
			...
			for (int j = 0; j < couplings.size(); j++) {
				GenCol.Pair pair = (GenCol.Pair) couplings.get(j);
	...
	protected class ContentPathStep {
		...
		/**
		 * At which port on the above view the content should be displayed for
		 * this step.
		 */
	...
	// /bpz
	public void modelViewPaint() {

		modelView.removeAllViews();

		modelView.createViews(model, modelView);
		// modelView.addCoupling(//);
		// couprel myCr = model.getCouprel();
		// modelView.restoreInternalCouplings(model,myCr);
		// detmCouplings( (ViewableDigraph) model);
		modelView.repaint();
	}
	...
	protected class ContentPathKey {
		...
		/**
		 * Equates the given object to this key if it is of the same class, if
		 * its content port and values match this key's, and if its component
		 * name matches this key's.
		 */
		...
		/**
		 * Returns a hash-code for this key that is based on a combination of
		 * its component-name and its content's port-name and value.
		 */
		public int hashCode() {
	...
	/**
	 * Saves the current top-level model's layout (within this sim-view) to its
	 * source code file.
	 */
	...
	/**
	 * Saves the given model's layout (within this sim-view) to its source code
	 * file.
	 */
	public void saveModelLayout(ViewableDigraph model) {
		...
		method.append("    /**\n");
		method
				.append("     * Automatically generated by the SimView program.\n");
		method
				.append("     * Do not edit this manually, as such changes will get overwritten.\n");
		method.append("     */\n");
		method.append("    public void layoutForSimView()\n");
		method.append("    {\n");
		method.append("ComputeLayout = false;\n");// bpz
		...
		while (i.hasNext()) {
			...
			Object next = i.next();
			if (!(next instanceof ViewableComponent)) {
				continue;
			}
			...
			method.append("        ");
			method.append("if (withName(" + component.getLayoutName()
					+ ")!=null)\n            ");
	...
	/**
	 * Saves the layouts (within this sim-view) of the children components of
	 * the given model to their associated source code files.
	 */

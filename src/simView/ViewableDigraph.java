...
{
    public boolean ComputeLayout = true;//bpz
    /**
     ...
     */
    public ViewableDigraph(){ //bpz 2007
        this("ViewableDigraph");
    }
     ...
     */
    public void addTestInput(String portName, GenCol.entity value) {addTestInput(portName, value, 0);}
     ...
     *                      injecting the value.
     */
    public void addTestInput(String portName, GenCol.entity value, double e) {
    ...
    public void updateComponentView(){
        if(myModelView==null) return; //Xiaolin Hu, Feb 20, 2006
        ...
        couprel savedCr;
        if(parent==null) savedCr = null; //Xiaolin Hu --Feb 18, 2006
        else{
            coordinator cd = parent.getCoordinator();
            savedCr = cd.getExternalModelCouplings(this);
        }
        ...
        while (cit.hasNext()){
            IODevs iod = (IODevs) cit.nextComponent();
            if (iod instanceof ViewableDigraph)
                ((ViewableDigraph) iod).updateComponentView();
        }
 ...
 */
    ...
    {
ComputeLayout = false;
        preferredSize = new Dimension(883, 451);
        if (withName("Deterministic")!=null)
             ((ViewableComponent)withName("Deterministic")).setPreferredLocation(new Point(315, 172));
    }
}
